    ...
    for list_filename in list_filenames:
        with pathmgr.open(list_filename, "r") as f:
            ...
            for line in csv_reader:
                row = line #comment for AVA dataset
                #row = line[0].split() #uncomment for AVA dataset
                #assert len(row)==5
                video_name = row[0]
...
def load_boxes_and_labels(cfg, mode):
    ...
    gt_lists = cfg.AVA.TRAIN_GT_BOX_LISTS if mode == "train" else cfg.AVA.TEST_PREDICT_BOX_LISTS
    # pred_lists = (
        ...
        with pathmgr.open(filename, "r") as f:
            for line in f:
                ...
                if video_name not in all_boxes:
                    ...
                    maxi = 0
                    with pathmgr.open(filename, "r") as file:
                        for line in file:
                            row = line.strip().split(",")
                            if row[0] == video_name:
                                if int(row[1])>maxi:
                                    maxi=int(row[1])
                    valid_frames = range(0,maxi+1)
                    #valid_frames = AVA_VALID_FRAMES
