...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta, _) in enumerate(train_loader):
        # image = inputs[0][:,:,8,:,:].squeeze().permute(1,2,0).cpu().numpy()
        # plt.figure()
        # plt.imshow(image)
        # plt.show()
        # bbox = meta['boxes'].cpu().numpy()[0]
        # imagebis= np.copy(image)
        # imagebis= cv2.rectangle(imagebis, (int(bbox[1]),int(bbox[2])),(int(bbox[1])+int(bbox[3]),int(bbox[2])+int(bbox[4])),color=(255, 0, 0), thickness=10)
        # plt.figure()
        # plt.imshow(imagebis)
        # plt.show()
        # breakpoint()
